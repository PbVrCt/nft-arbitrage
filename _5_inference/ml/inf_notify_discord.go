...
func notify_discord(nft AxieInfoEngineered, current_prices []float64, price_history PriceHistory) {
	var important_stat float64
	switch class := nft.Class; class {
	case "Bird", "Aquatic", "Dawn":
		important_stat = nft.Speed
	case "Plant", "Reptile", "Dusk", "Bug":
		important_stat = nft.Hp
	case "Beast", "Mech":
		important_stat = nft.Morale
	}
	...
	embed := &discordgo.MessageEmbed{
		...
		Fields: []*discordgo.MessageEmbedField{
			...
			{
				Name:   "Important stat",
				Value:  fmt.Sprintf("%.2f", important_stat),
				Inline: true,
			},
			// {
			// 	Name:   "Stats",
