...
func CreateBodySimilarNfts(nft AxieInfo, size int, combo map[string]string) RequestBody {
	body := RequestBody{}
	...
	var parts []string
	switch {
	case reflect.DeepEqual(combo, map[string]string{}):
		parts = []string{fmtPart(nft.Back, "back"), fmtPart(nft.Mouth, "mouth"), fmtPart(nft.Horn, "horn"), fmtPart(nft.Tail, "tail")}
	default:
		for key, value := range combo {
			parts = append(parts, fmtPart(value, key))
		}
	}
...
// PARSE RESPONSES SIMILAR NFTS

func ParsePricesSimilarNfts(data []byte) []float64 {
	var result JsonBlob
...
// DEFINE REQUESTS BASED ON NFT PARTS
func CreateBodyFilterByParts(from int, query string, parts []NftPart) RequestBody {
	body := RequestBody{}
	body.Query = query
	body.Variables.From = from
	body.Variables.Size = 100
	...
	var sorting string = "PriceAsc"
	body.Variables.Sort = &sorting
	var parts_fmt []string
	for _, NftPart := range parts {
		parts_fmt = append(parts_fmt, fmtPart(NftPart.Part, NftPart.Parttype))
	}
	body.Variables.Criteria.Parts = &parts_fmt
	var Stages int = 4
	body.Variables.Criteria.Stages = &Stages
	return body
}
